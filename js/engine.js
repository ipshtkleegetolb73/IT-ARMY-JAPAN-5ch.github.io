...
//Controla el bot髇 para pausar el ataque.
function start_stop(){
	if (document.getElementById('state').innerHTML == 'stop') {
		...
		fire = setInterval('shoot()', $('#sl0').mbgetVal() / 10);
	}
	else {
...
function score_tail(){
	//-----------------------------------------------------
	//Por ahora no est� lista la funci髇 de atacar varias
	...
	document.getElementById('counter_tail').innerHTML = parseInt(tail) + 1//parseInt(calibre);
}
